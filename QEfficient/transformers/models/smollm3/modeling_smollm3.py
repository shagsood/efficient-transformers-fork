...
class QEffSmolLM3RotaryEmbedding(SmolLM3RotaryEmbedding):
...
class QEffSmolLM3Attention(SmolLM3Attention):
    def forward(
        self,
        ...
        **kwargs,
        ...
        query_shape = (batch_size, query_length, self.config.num_attention_heads, self.head_dim)
        key_value_shape = (batch_size, query_length, self.config.num_key_value_heads, self.head_dim)
        ...
        attn_output = attn_output.reshape(batch_size, query_length, self.config.num_attention_heads * self.head_dim)
        attn_output = self.o_proj(attn_output)
        return attn_output, attn_weights


class QEffSmolLM3DecoderLayer(SmolLM3DecoderLayer):
    def forward(
        self,
        hidden_states: torch.Tensor,
...
class QEffSmolLM3Model(SmolLM3Model):
    def __qeff_init__(self):
        self.rotary_emb = QEffSmolLM3RotaryEmbedding(config=self.config)
...
class QEffSmolLM3ForCausalLM(SmolLM3ForCausalLM):
    def get_submodules_for_export(self) -> type[nn.Module]:
        return {QEffSmolLM3DecoderLayer}
